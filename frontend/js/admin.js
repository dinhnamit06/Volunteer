        ...
        if (listContainer) {
            ...
            listContainer.innerHTML = events.map(e => `
                <div class="stat-card mb-3" style="display:flex; justify-content:space-between; align-items:center; padding:15px">
                    ...
                    <div>
                        <button onclick="openEditEventModal(${e.id})" style="background:var(--warning); color:white; border:none; padding:5px 10px; border-radius:5px; cursor:pointer; font-size:12px; margin-right:5px">Sửa</button>
...
function openEditEventModal(id) {
    ...
    document.getElementById('edit-event-name-display').textContent = event.title;
    document.getElementById('edit-event-title').value = event.title || '';
    document.getElementById('edit-event-code').value = event.activity_code || '';
    document.getElementById('edit-event-org').value = event.org_level || 'Trường';
    document.getElementById('edit-event-type').value = event.activity_type || '';
    document.getElementById('edit-event-nature').value = event.activity_nature || 'Tự nguyện';
    document.getElementById('edit-event-location').value = event.location || '';

    // Format dates to YYYY-MM-DDTHH:MM if they exist
    ...
    document.getElementById('edit-event-start').value = formatDateTimeLocal(event.start_time);
    document.getElementById('edit-event-end').value = formatDateTimeLocal(event.end_time);

    document.getElementById('edit-event-capacity').value = event.capacity || '';
    document.getElementById('edit-event-audience').value = event.target_audience || '';
    document.getElementById('edit-event-tnv').value = event.ecc_points_volunteer || 0;
    document.getElementById('edit-event-btc').value = event.ecc_points_organizer || 0;
...
async function submitEditEvent(e) {
    ...
    const payload = {
        ...
        activity_code: document.getElementById('edit-event-code').value,
        org_level: document.getElementById('edit-event-org').value,
        activity_type: document.getElementById('edit-event-type').value,
        activity_nature: document.getElementById('edit-event-nature').value,
        ...
        target_audience: document.getElementById('edit-event-audience').value,
        ecc_points_volunteer: parseFloat(document.getElementById('edit-event-tnv').value) || 0,
        ecc_points_organizer: parseFloat(document.getElementById('edit-event-btc').value) || 0,
        created_by: currentUser.id
    };
    ...
    try {
        ...
        if (res.ok) {
            showToast("Thành công", "Đã cập nhật sự kiện", "success");
            closeEditEventModal();
            loadEvents();
            loadStats();
        } else {
...
async function promoteStaff(userId) {
    ...
    try {
        const res = await fetch('http://localhost:5000/api/admin/promote-staff', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
